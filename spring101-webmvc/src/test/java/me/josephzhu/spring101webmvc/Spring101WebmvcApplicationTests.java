...
public class Spring101WebmvcApplicationTests {
    ...
    public void testExceptionApiController() throws Exception {
        webTestClient.get().uri("/exception/item/1").exchange().expectStatus().isOk().expectBody().json("{'data':{'name':'item1','price':10}}");
        webTestClient.get().uri("/exception/item/aa").exchange().expectStatus().isOk().expectBody().json("{'code':'1001','message':'商品ID只能是数字'}");
        webTestClient.get().uri("/exception/item/0").exchange().expectStatus().isOk().expectBody().json("{'code':'500','message':'不合法的商品ID'}");
        webTestClient.post().uri("/exception/item").syncBody(new MyItem()).exchange().expectStatus().isOk().expectBody().json("{'code':'422','error':'Unprocessable Entity'}");
    ...
    public void testFilterApiController() throws Exception {
        ...
        webTestClient.get().uri("/filter/hello").exchange().expectStatus().isOk().expectBody().json("{'message':'请登录！'}");
